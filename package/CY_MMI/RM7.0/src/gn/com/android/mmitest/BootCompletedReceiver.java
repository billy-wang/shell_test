...
public class BootCompletedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Log.i(TAG, "action=" + action + ";isFirstBoot=" + isFirstBoot + ";isMtklogRunning=" + isMtklogRunning);
        ...
        if (list != null) {
            for (int j = 0; j < list.size(); j++) {
                if ("com.mediatek.mtklogger".equals(list.get(j).processName)
                    ...
                    //Log.i(TAG, "processName "+j+"="+list.get(j).processName+";importance="+list.get(j).importance);
                    isRunning = true;
                    break;
                }
            }
    ...
    private boolean isMtklogExist() {
        ...
        Log.i(TAG, "isMtklogExist=" + isMtklogExist);
    ...
    private void DeleteMtkLog() {
        try {
            ...
            Log.e(TAG, "DeleteMtkLog error=" + e.getMessage());
        }
    }
