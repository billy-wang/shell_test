...
import gn.com.android.mmitest.item.SensorTest.DataType;
...
public class SensorID {
    ...
    /**
     * Available sensor types. Values of each type are defined in
     * "sns_smgr_api_v01.h"
     */
    public enum SensorType {
        ACCEL(0), GYRO(10), MAG(20), PRESSURE(30), PROX_LIGHT(40);
    ...
    /**
     * @param sensorType
     *            The type of sensor for which we are constructing an ID.
     * @param sensorCount
     *            If there are multiple of the sensor type on the device, the
     *            one we wish to query (0 for primary). As presently defined, at
     *            most 9 instances of any given sensor type can co-exist on a
     *            single device.
     * @throws IllegalArgumentException
     *             Range Exception on sensorCount, or unsupported sensor type
     */
    public SensorID(SensorType sensorType, int sensorCount) throws IllegalArgumentException {
        if (sensorCount > 9 || sensorCount < 0)
            throw new IllegalArgumentException(
                    "Range Exception: the value of sensor count must be between 0 and 9, inclusive");
        if (sensorType == null) {
