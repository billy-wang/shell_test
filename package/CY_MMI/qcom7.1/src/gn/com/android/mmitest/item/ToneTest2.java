...
public class ToneTest2 extends Activity implements OnClickListener {
    Button mToneBt;
    ...
    private final byte generatedSed[] = new byte[2 * 2 * numSamples];
    ...
    public void onResume() {
        ...
        if (null != mAM) {
            ...
            String aString = TestUtils.setStreamVoice("ToneTest2");
    ...
    void genTone() {
        ...
        for (int ids = 1; ids < numSamples; ids = ids + 2) {
            if (ids < (numSamples - 2)) {
                generatedSed[2 * (ids - 1)] = generatedSnd[ids - 1];
                generatedSed[2 * (ids - 1) + 1] = generatedSnd[ids];
                generatedSed[2 * (ids - 1) + 2] = 0;
                generatedSed[2 * (ids - 1) + 3] = 0;
            }
        }
    }

    void playSound() {
        mAudioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, sampleRate, AudioFormat.CHANNEL_CONFIGURATION_STEREO,
                AudioFormat.ENCODING_PCM_16BIT, numSamples, AudioTrack.MODE_STATIC);
        mAudioTrack.write(generatedSed, 0, generatedSed.length);
        mAudioTrack.play();
