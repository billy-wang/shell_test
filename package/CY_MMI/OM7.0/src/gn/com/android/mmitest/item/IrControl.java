...
public class IrControl {
    ...
    public IrControl(Context ctx){
        ...
        if(objir == null) DswLog.d("RemoteIr", "No RemoteIR");
        if(method_transmit == null) DswLog.d("RemoteIr", "No Transmit Method");
        if(method_stop == null) DswLog.d("RemoteIr", "No Stop Method");
         if(method_receive == null) DswLog.d("RemoteIr", "No received Method");
        if(method_receive_init == null) DswLog.d("RemoteIr", "No receive_init Method");
         if(method_receive_is_ready == null) DswLog.d("RemoteIr", "receive_is_ready");
        //if(method_stop == null) DswLog.d("RemoteIr", "No Stop Method");
    }
    ...
    public int Recevied_Init()
    {
        ...
        {
            DswLog.e("RemoteIr recevied init", e.toString());
        }
    ...
    public int ReceivedIsRead()
    {
        ...
        {
            DswLog.e("RemoteIr recevied is read method", e.toString());
        }
    ...
    public int RecviedIR(byte[] buffer,int i32MaxLength)
    {
        ...
        {
            DswLog.e("RemoteIr reccevie method", e.toString());
        }
    ...
    public int[] ReaciveDataAnlazy(byte[] buffer,int i32Length)
    {
        ...
        i32Length = tme;//buffer[1];
    	   if(buffer[0] != 0x00)
           {
               DswLog.e("DataAna ", "packet no err");
               return new int[0];
           }
    	   if(i32Length<30)
           {
               DswLog.e("DataAna ", "Data Length err");
               return new int[0];
           }

          chk1 = buffer[i32Length];
            chk1 &= 0x00FF;
        DswLog.e("DataAna Checksum H : ", ""+chk1);
        chk1 <<= 8;
           tme = buffer[i32Length+1];
            tme &= 0x00FF;

        DswLog.e("DataAna Checksum L : ", ""+tme);
           chk1 += tme;

        chk2 = 0;
        for(i=0;i<(i32Length+2);i++)
           DswLog.e("DataAna",""+buffer[i]);
        ...
        for(i=0;i<(i32Length);i++)
           ...
           if(chk2!=chk1) {
               DswLog.e("DataAna checksum", "chk1!=chk2");
               return new int[0];
           }
    	    ArrayList<Integer> frameList = new ArrayList<Integer>();
