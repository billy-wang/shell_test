    ...
    public void onStart() {
      ...
      /*  PhoneFactory.getDefaultPhone().getMobileRevisionAndImei(5,
        ...
        if (FeatureOption.BACKUP_TO_PRODUCTINFO) {
            updateSN();
            Log.e(TAG, "TestResult please turn on MTK_PRODUCT_INFO_SUPPORT == false");
        } else {
            Toast.makeText(this, "please turn on MTK_PRODUCT_INFO_SUPPORT", Toast.LENGTH_LONG).show();
            Log.e(TAG, "TestResult please turn on MTK_PRODUCT_INFO_SUPPORT == true");
        }
        //Gionee zhangke 20151013 modify for CR01562456 end
        Log.e(TAG, "TestResult FeatureOption.BACKUP_TO_PRODUCTINFO");
        ...
        if (null != binder) {
            ...
            try {
                ...
                for (int i = 0; i < SN_LENGTH; i++) {
                    Log.i(TAG, "before write_buff[" + i + "]=" + write_buff[i]);
                }
                ...
                if (flag > 0) {
                    ...
                    for (int i = 0; i < SN_LENGTH; i++) {
                        Log.i(TAG, "after write_buff[" + i + "]=" + write_buff[i]);
                    }
