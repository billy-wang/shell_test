...
public class SimCardAndTF extends BaseActivity implements OnClickListener ,OnPerformListen{
	...
	private boolean isTFPass = false;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tvTitle.setText(R.string.sim_sdcard_title);

		mRightBtn.setEnabled(false);
		mWrongBtn.setEnabled(false);
		mRestartBtn.setEnabled(false);

		mRightBtn.setOnClickListener(SimCardAndTF.this);
		mWrongBtn.setOnClickListener(SimCardAndTF.this);
		mRestartBtn.setOnClickListener(SimCardAndTF.this);
	}
	...
	private void onSimCardTest() {
		...
		tvContent.setText(mResult);
		DswLog.d(TAG, "SimCard result=" + result + " simSlotNum="+simSlotNum);
		isSimPass = ProinfoUtil.isTestDoubleSim() ? (sim1_status && sim2_status):(sim1_status || sim2_status);
	}
	...
	private void onSdcardTest() {
		String result = "TFCard:";
		DswLog.d(TAG,"onSdcardTest");
		...
		if (isTFPass) {
			result = result + getString(R.string.sim_sdcard_sim_well);
		}else {
			result = result + getString(R.string.sim_sdcard_sim_none);
		}
		result = result + info.toString();
		//info = info.substring(0, info.length()-1);
		DswLog.d(TAG, "TFCard result=" + result);
		mResult = result;
		tvContent.setText(mResult);
	}
