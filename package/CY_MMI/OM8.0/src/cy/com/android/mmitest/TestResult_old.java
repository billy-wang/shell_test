...
import cy.com.android.mmitest.item.NvRAMBackupAgent;
...
public class TestResult extends BaseActivity {
  ...
  //  public static final int ALL_LENGTH = 1024;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            mmiCtx = createPackageContext("cy.com.android.mmitest", Context.CONTEXT_IGNORE_SECURITY | Context.CONTEXT_INCLUDE_CODE);
    ...
    public static int writeToProductInfo(byte[] sn_buff) {
        ...
        if (null != binder) {
            ...
            try {
                byte[] write_buff = agent.readFileByName(ProinfoUtil.getProinfoPath());
                ...
                int flag = agent.writeFileByName(ProinfoUtil.getProinfoPath(), write_buff);
                if (flag > 0) {
                    DswLog.e(TAG, "writeToProductInfo NvRAMAgent write success");
                    write_buff = agent.readFileByName(ProinfoUtil.getProinfoPath());
                    for (int i = 0; i < SN_LENGTH; i++) {
    ...
    private void updateSN() {
        try {
            System.arraycopy(readINvramInfo(), 0, mSnByteArray, 0, SN_LENGTH);
