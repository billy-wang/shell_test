...
/*****************************************************************************
 * ZCalibView.java - ZCalib Application Example Copyright (c) 2013-2017 HiDeep,
...
public class ZCalibView extends View {
    ...
    //Gionee zhangke 20160108 modify for CR01618870 start
    private static final int MAX_TOUCH_NUM = 10;// 2
    //Gionee zhangke 20160108 modify for CR01618870 end
    ...
    public void setPressure_Z(float m_Pressure_Z) {
        this.m_Pressure_z = m_Pressure_Z;
    }

    public float getPressure_Z() {
        return this.m_Pressure_z;
    }
        ...
        for (int pressId = 0; pressId < this.m_PSensor.getPressLenth(); pressId++) {
            if (this.m_PSensor.getPress(pressId)) {
                ...
                for (int i = 0; i < this.m_PressureCirclePaint.length; i++) {
                    ...
                    if (drawCircleSize > 0) {
                        canvas.drawCircle(this.m_CircleX[pressId], this.m_CircleY[pressId], drawCircleSize,
                                this.m_PressureCirclePaint[i]);
    ...
    private void onDrawText(Canvas canvas) {
        for (int pressId = 0; pressId < this.m_PSensor.getPressLenth(); pressId++) {
            if (this.m_PSensor.getPress(pressId)) {
                ...
                //Gionee zhangke 20151214 modify for CR01591381 start
                float gramme = this.m_Pressure * 1000;
                //Gionee zhangke 20160108 add for CR01619816 start
                if(gramme >= 995f && gramme <= 1000f){
                    gramme = 1000f;
                }
                //Gionee zhangke 20160108 add for CR01619816 end
                String strNum = String.format("%.2f", gramme);
                if (this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_CAL_POINT) {
                    strNum = tmpPrs;
                } else {
                    strNum = strNum +"g" + " (" + tmpPrs + ")";
                }
                //Gionee zhangke 20151214 modify for CR01591381 end
    ...
    private boolean CheckScreen() {
        if (this.m_ScreenX > 0 && this.m_ScreenY > 0) {
            return true;
        } else {
            return false;
        }
    ...
    private void SetGridLine(Canvas canvas) {
        /**
         * to comment if (this.CheckScreen() == true) { float hgridMargin =
         * (this.m_ScreenX) / ZCALIB_GRID_GOUNT_X; float vgridMargin =
         * (this.m_ScreenY) / ZCALIB_GRID_GOUNT_Y;
         * 
         * for (int i = 0 ; i <= ZCALIB_GRID_GOUNT_X ; i++) { canvas.drawLine(
         * (float)(i * hgridMargin) + (ZCALIB_GRID_WIDTH / 2), (float)0,
         * (float)(i * hgridMargin) + (ZCALIB_GRID_WIDTH / 2),
         * (float)this.m_ScreenY, this.m_PaintGrid); }
         * 
         * for (int i = 0; i <= ZCALIB_GRID_GOUNT_Y ; i++) { canvas.drawLine( 0,
         * (float)(i * vgridMargin) + 2, (float)this.m_ScreenX, (float)(i *
         * vgridMargin) + 2, this.m_PaintGrid); } }
         */
    }
            ...
            for (int i = 0; i < ZCALIB_GRID_GOUNT_Y; i++) {
                for (int j = 0; j < ZCALIB_GRID_GOUNT_X; j++) {
                    if (k == this.m_CurrentGUICalPoint) {
                        ...
                        if (this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_CAL_POINT) {
                            if (this.m_ViewActivity
                                    .getCurrentZCalibGetteringStatus() == ZCalib.STATUS_GETTERING_CAL_VALUE_SAVE_START) {
                                tmpPaintRect = this.m_PaintGridRect2;
                            } else if (this.m_ViewActivity
                                    .getCurrentZCalibGetteringStatus() == ZCalib.STATUS_GETTERING_CAL_VALUE_SAVE_FINISH) {
                    ...
                    if (tmpPaintRect != null) {
                        if (this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_CAL_POINT) {
                            canvas.drawRect((float) j * hgridMargin + 1, (float) i * vgridMargin + 1,
                                    (float) (j + 1) * hgridMargin - 1, (float) (i + 1) * vgridMargin - 1, tmpPaintRect);
                        } else if (this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_VERIFY_POINT) {
                            if (k == this.m_CurrentGUICalPoint) {
                                canvas.drawRect(0, 0, this.m_ScreenX, this.m_ScreenY, tmpPaintRect);
                            }
                        }
                    }
                    k++;
    ...
    private void SetSpecResult(Canvas canvas) {
        ...
        // verify points
        if (this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_VERIFY_POINT) {
            this.m_VerifyPointXYPosition.set(this.m_ViewActivity.getCurrentZCalibPointIdx(),
                    new float[] { this.m_CurrentX, this.m_CurrentY });
        }

        for (int i = 0; i < this.m_ZCalibVerifyPointCount; i++) {
            if ((this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_VERIFY_POINT
                    && i < this.m_CurrentGUICalPoint)
                    || this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_RESULT
                ...
                try {
                    ...
                    if (tmp == ZCalib.RESULT_PASS || tmp == ZCalib.RESULT_NG) {
                        ...
                        canvas.drawText(strSpecResult, m_VerifyPointXYPosition.get(i)[0],
                                m_VerifyPointXYPosition.get(i)[1], tmpPaint);

                        if (tmp == ZCalib.RESULT_NG) {
                            DecimalFormat df = new DecimalFormat("0.00");
                            strSpecValue = "" + df.format(this.m_ViewActivity.getZCalibVerificationSpecResultArray(i))
                                    + "%";
                            this.m_PaintGridSpecValueText.getTextBounds(strSpecValue, 0, strSpecValue.length(),
                                    bounds2);
                            canvas.drawText(strSpecValue, m_VerifyPointXYPosition.get(i)[0],
                                    m_VerifyPointXYPosition.get(i)[1] + 30, this.m_PaintGridSpecValueText);
                        }
                    }
    ...
    private void SetDrawVerifyNumber(Canvas canvas) {
        ...
        canvas.drawText(str, fX, fY, this.m_PainTextNumber51);
    }
                ...
                for (int j = 0; j < ZCALIB_GRID_GOUNT_X; j++) {
                    ...
                    if (tmpPaint != null) {
                        if (this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_CAL_POINT
                            ...
                            // Number
                            // if
                            // (this.m_ZCalibValues.getZCalibSpecResult(nCount -
                            // 1) == 0)
                            // {
                            ...
                            canvas.drawText(strConvCount,
                                    ...
                                    (float) ((i * vgridMargin) + (vgridMargin / 2) + (bounds.height() / 2.)), tmpPaint);
                            // }
                        }
                    }
                    k++;
     ...
     * this.m_PaintCheckedGridRect); canvas.drawLine(rect.left, rect.bottom,
     * rect.right, rect.top, this.m_PaintCheckedGridRect); } }
     */
    ...
    /**
     * touched pos. is the same cal point.
     * 
     * @param x
     * @param y
     */
    public void SearchTouchedPosEqualsCurrentIdx(float x, float y) {
        if (this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_CAL_POINT
                && this.m_ViewActivity.getCurrentZCalibPointIdx() != this.SearchTouchedNumber(x, y) - 1) {
            this.m_ViewActivity.GoToDisplayNG();
            postInvalidate();
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_DOWN: {
            ...
            this.SearchTouchedPosEqualsCurrentIdx(x, y);
            break;
        }

        case MotionEvent.ACTION_MOVE: {
            // int maxCnt = event.getPointerCount() >= MAX_TOUCH_NUM ?
            // MAX_TOUCH_NUM : event.getPointerCount();
            int totalPress = 0;
        ...
        case MotionEvent.ACTION_UP: {
            ...
            // int nCheckedNum =
            // this.SearchTouchedNumber((int)event.getX(pointer_index),
            // (int)event.getY(pointer_index));
            if (this.m_ViewActivity.getCurrentZCalibStatus() == ZCalib.STATUS_CURRENT_CAL_POINT) {
