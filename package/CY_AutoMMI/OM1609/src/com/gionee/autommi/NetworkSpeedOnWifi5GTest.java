...
public class NetworkSpeedOnWifi5GTest extends BaseActivity {
	...
	private void setupWifi() {
        ...
        Log.d(TAG, "disable wifi ,  then enable ...");
        try {
                Thread.sleep(500);
        } catch (InterruptedException e) {
                    System.out.println(e);
	...
	private void downLoad() {
		try {
			...
			try {
				...
				Log.d(TAG, "IOException #1 error break");
				e.printStackTrace();
			} finally {
				if (null != os)
					try {
						...
						Log.d(TAG, "IOException #2 error break");
						e.printStackTrace();
					}
			}
		} catch (MalformedURLException e) {
			Log.d(TAG, "MalformedURLException error break");
			e.printStackTrace();
		}
		//Gionee <GN_BSP_MMI> <chengq> <20170424> modify for ID 121459 begin
		catch (NullPointerException e) {
			Log.d(TAG, "NullPointerException error break");
			e.printStackTrace();
		}
