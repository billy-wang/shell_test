...
//
import com.gionee.autommi.SensorUserCal;
...
public class SensorsCalTest extends BaseActivity implements SensorEventListener {
	...
	static {
        System.loadLibrary("sensor_user_cal2");
    }
    ...
    protected void onStart() {
        ...
        super.onStart();
        if(FeatureOption.GN_RW_GN_MMI_SENSOR_COMPASS_SUPPORT){
            mSensorMgr.registerListener(this, mMSensor, SensorManager.SENSOR_DELAY_NORMAL);
        }
    ...
    private void doPSensorCalibration() {
        ...
        int result = SensorUserCal.performUserCal((byte)40,(byte)0);
        if (result == 0) {
    ...
    private void doGSensorCalibration() {
        ...
        int result = SensorUserCal.performUserCal((byte)0,(byte)0);
        if (result == 0) {
    ...
    private void doGyroscopeCalibration() {
        ...
        int result = SensorUserCal.performUserCal((byte)10,(byte)0);
    ...
    /**
     * Performs calibration on the applicable sensor and immediately applies the
     * results.
     *
     * @param sensorType
     *            Defines the sensor that this configuration pertains to. The
     *            sensor can be one of following: 00 - SNS_SMGR_ID_ACCEL 10 -
     *            SNS_SMGR_ID_GYRO 20 - SNS_SMGR_ID_MAG 30 -
     *            SNS_SMGR_ID_PRESSURE 40 - SNS_SMGR_ID_PROX_LIGHT
     * @param dataType
     *            Defines sensor data type which classifies if the data type is
     *            primary or secondary. 00 - SNS_SMGR_DATA_TYPE_PRIMARY 01 -
     *            SNS_SMGR_DATA_TYPE_SECONDARY
     * @return 0: No error -1: Response error - timeout -2: Sensor1 Error -3:
     *         Error using bias -4: Error writing to registry
     */
    static public native synchronized int performPsensorCal(byte sensorType, byte dataType);
