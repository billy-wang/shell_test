...
public class FlagTrustonic extends BaseActivity {

    public static final String TAG = "FlagTrustonic";
    private int presult;
    private static final int SN_LENGTH = 501;
    private static final int MMI_TR_FLAG = 500;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        presult = this.getIntent().getIntExtra("TrustoFlag", -1);
    }
    ...
    private void startWriteFlag() {
        ...
        mSnByteArray = SystemUtil.getNewSN(MMI_TR_FLAG, flag, mSnByteArray);
    ...
    private boolean isWritePass(int snLength) {
        ...
        if (productInfoBuff != null && productInfoBuff.length > MMI_TR_FLAG) {
            DswLog.d(TAG, "FlagTrustonic: isWritePass[500]=" + productInfoBuff[MMI_TR_FLAG]);
            if ('P' == productInfoBuff[MMI_TR_FLAG])
                return true;
        }
        return false;
    }
            ...
            switch (msg.what) {
                case 1:
                    if (!isWritePass(SN_LENGTH)) {
                        DswLog.d(TAG, "write FlagTrustonic Failed");
                        return;
                    }
