package com.cydroid.autommi.EmSensor;
...
public class EmSensor {
	...
	private void initClassLoader() {
		try {
			...
			DswLog.v(TAG, "mClassName:" + mClass.getName());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public int doGSensorCalibration() {
		...
		try {
			...
			DswLog.v(TAG, "doGSensorCalibration: result = " + ret);
		}
		return ret;
	}
	...
	private int doPsensorCalibration(int min, int max) {
		...
		try {
			...
			DswLog.v(TAG, "doPsensorCalibration: result = " + result);
		}
		return result;
	}
	...
	public int getPsensorData() {
		...
		try {
			...
			DswLog.v(TAG, "getPsensorData: result = " + result);
		}
		return result;
	}
	...
	private int getPsensorThreshold(int[] result) {
		...
		try {
			...
			DswLog.v(TAG, "getPsensorThreshold: ret = " + ret +". Threshold:"+ result[0]
					+ " " + result[1]);
		}
		return ret;
	}
	...
	private int getPsensorMaxValue() {
		...
		try {
			...
			DswLog.v(TAG, "getPsensorMaxValue: result = " + result);
		}
		return result;
	}
	...
	private int getPsensorMinValue() {
		...
		try {
			...
			DswLog.v(TAG, "getPsensorMinValue: result = " + result);
		}
		return result;
	}
